...
export function renderCategoryChart(transactions) {
    ...
    categoryChartInstance = new Chart(ctx, {
        type: 'doughnut',
        data: { labels, datasets: [{ data: amounts, backgroundColor: ['#4CAF50','#FF9800','#2196F3','#F44336','#9C27B0','#FFC107'] }] },
...
export function renderBalanceChart(transactions) {
    ...
    balanceChartInstance = new Chart(ctx, {
        ...
        options: { responsive: true, plugins: { legend: { display: true }, title: { display: true, text: 'Balance Trend' } }, scales: { y: { beginAtZero: true } } }
    });
}
