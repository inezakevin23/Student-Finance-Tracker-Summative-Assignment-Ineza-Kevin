...
export function renderCategoryChart(transactions) {
    ...
    categoryChartInstance = new Chart(ctx, {
        type: 'doughnut',
        data: { labels, datasets: [{ data: amounts, backgroundColor: ['#F44336','#FF9800','#2196F3','#4CAF50','#9C27B0','#FFC107'] }] },
        options: { 
            responsive: true, 
...
export function renderIncomeCategoryChart(transactions) {
    ...
    incomeCategoryChartInstance = new Chart(ctx, {
        type: 'doughnut',
        data: { labels, datasets: [{ data: amounts, backgroundColor: ['#4CAF50','#2196F3','#FF9800','#F44336','#9C27B0','#FFC107'] }] },
        options: { 
            responsive: true, 
            maintainAspectRatio: false,
            plugins: { 
...
export function renderBalanceChart(transactions) {
    ...
    balanceChartInstance = new Chart(ctx, {
        type: 'line',
        data: { labels: dataPoints.map(d => d.date), datasets: [{ label: 'Balance Over Time', data: dataPoints.map(d => d.balance), fill: false, borderColor: '#2196F3', tension: 0.1 }] },
        options: { 
            responsive: true, 
            maintainAspectRatio: false,
            plugins: { legend: { display: true }, title: { display: true, text: 'Balance Trend' } }, 
