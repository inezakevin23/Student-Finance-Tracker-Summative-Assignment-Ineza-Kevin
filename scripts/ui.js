...
function getPageType() {
  ...
  if (document.getElementById("transactions")) return "transactions";
  if (document.getElementById("settings")) return "settings";
  if (
...
// --- Format amount in chosen currency ---
...
function renderStats(stats) {
  ...
  statsGrid.innerHTML = `
        <div class="stat-card">
            <h3>Balance</h3>
            <p class="stat-value balance">${formatAmount(
              stats.balance.toFixed(2)
            )}</p>
        </div>
        <div class="stat-card">
            <h3>Income</h3>
            <p class="stat-value income">${formatAmount(
              stats.income.toFixed(2)
            )}</p>
        </div>
        <div class="stat-card">
            <h3>Expenses</h3>
            <p class="stat-value expense">${formatAmount(
...
function renderRecent(transactions) {
  ...
  transactions
    ...
    .forEach((tx) => {
      ...
      li.textContent = `${tx.date}: ${tx.description} (${tx.category}) ${
        tx.type === "income" ? "+" : "-"
...
function setupForms(updateUI) {
  document.getElementById("show-income-form").onclick = () =>
    showSection("add-income");
  document.getElementById("show-expense-form").onclick = () =>
...
// --- Currency change and manual exchange rates ---
function setupCurrencySettings(updateUI) {
  ...
  if (curSelect) {
    curSelect.innerHTML = state.settings.availableCurrencies
      .map(
        (c) =>
          `<option value="${c}" ${
            state.settings.preferredCurrency === c ? "selected" : ""
          }>${c}</option>`
      )
      .join("");
  ...
  if (rateContainer) {
    rateContainer.innerHTML = state.settings.availableCurrencies
      .map((currency) => {
        return `
            <div>
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (page === "dashboard") {
    renderStats(calculateStats(records.getTransactions()));
    renderCategoryChart(records.getTransactions());
    renderBalanceChart(records.getTransactions());
    renderIncomeCategoryChart(records.getTransactions());
    renderRecent(records.getTransactions());
    // About section does not require JS
  }

  if (page === "transactions") {
    setupForms(updateUI);
    setupSearchAndSort(updateUI);
    setupTransactionActions(records.getTransactions(), updateUI);
    ...
    function updateUI() {
      ...
      const { filtered, highlight } = filterAndHighlight(
        txs,
        pattern,
        caseInsensitive
      );
